//! Split Protovalidate message-level CEL from synthesized `protobuf` fences.
//!
...
"#;
    ...
    #[test]
    fn splits_experiment_spec_map_reduce_cel() {
        let body = r#"message ExperimentSpec {
  option (buf.validate.message).cel = {
    id: "experiment.arms_weights_sum"
    message: "arm weights must sum to at most 10000 basis points"
    expression:
        "this.arms.map(a, a.weight_basis_points).reduce(a, b, a + b, 0u) <= 10000u"
  };
  string experiment_id = 1;
}
"#;
        let (body, blocks) = split_message_cel_blocks(body);
        assert_eq!(blocks.len(), 1);
        assert!(!body.contains("10000u"));
        assert!(blocks[0].contains("experiment.arms_weights_sum"));
    }

    #[test]
    fn splits_pipeline_run_multiline_expression() {
        let body = r#"message PipelineRun {
  option (buf.validate.message).cel = {
    id: "pipeline_run.completed_after_started"
    message: "completed_at must not precede started_at when both are set"
    expression:
        "!has(this.completed_at) || !has(this.started_at) || this.completed_at >= this.started_at"
  };
  string run_id = 1;
}
"#;
        let (body, blocks) = split_message_cel_blocks(body);
        assert_eq!(blocks.len(), 1, "body={body}");
        assert!(!body.contains("pleted_at"), "body={body}");
        assert!(blocks[0].contains("pipeline_run.completed_after_started"));
    }
